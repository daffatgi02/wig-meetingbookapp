                ...
                <div class="card-body">
                    @if ($booking->requires_reapproval)
                        <div class="alert alert-warning">
                            ...
                            <strong>Note:</strong> Changes to this booking will require admin approval again.
                        </div>
                    @endif
                    ...
                    <form action="{{ route('bookings.update', $booking) }}" method="POST" id="editBookingForm">
                        ...
                        <!-- Include the same form fields as create.blade.php but with values -->
                        <!-- Basic Information -->
                        <div class="row mb-4">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('purpose') is-invalid @enderror" id="purpose"
                                    ...
                                    <option value="Meeting"
                                        {{ old('purpose', $booking->purpose) === 'Meeting' ? 'selected' : '' }}>Meeting
                                    </option>
                                    <option value="Training"
                                        {{ old('purpose', $booking->purpose) === 'Training' ? 'selected' : '' }}>Training
                                    </option>
                                    <option value="Presentation"
                                        {{ old('purpose', $booking->purpose) === 'Presentation' ? 'selected' : '' }}>
                                        Presentation</option>
                                    <option value="Workshop"
                                        {{ old('purpose', $booking->purpose) === 'Workshop' ? 'selected' : '' }}>Workshop
                                    </option>
                                    <option value="Conference"
                                        {{ old('purpose', $booking->purpose) === 'Conference' ? 'selected' : '' }}>
                                        Conference</option>
                                    <option value="Interview"
                                        {{ old('purpose', $booking->purpose) === 'Interview' ? 'selected' : '' }}>Interview
                                    </option>
                                    <option value="Other"
                                        {{ old('purpose', $booking->purpose) === 'Other' ? 'selected' : '' }}>Other</option>
                                </select>
                                @error('purpose')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-4">
                            <div class="col-md-8">
                                ...
                                <!-- Room Info Display -->
                                <div id="roomInfo" class="mt-2 p-3 bg-light rounded">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <small class="text-muted">
                                                <i class="fas fa-map-marker-alt me-1"></i>
                                                <span id="roomLocation">{{ $booking->room->location }}</span>
                                            </small>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted">
                                                <i class="fas fa-users me-1"></i>
                                                Capacity: <span id="roomCapacity">{{ $booking->room->capacity }}</span>
                                                people
                                            </small>
                                        </div>
                                    </div>
                                    <div class="mt-2">
                                        <small class="text-muted">
                                            ...
                                            Facilities: <span
                                                id="roomFacilities">{{ implode(', ', $booking->room->facilities ?? []) }}</span>
                                        </small>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number"
                                    ...
                                    value="{{ old('participant_count', $booking->participant_count) }}" required
                                    min="1" max="100">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                rows="3" placeholder="Any additional information about the meeting...">{{ old('description', $booking->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                        ...
                                        <div class="participant-item mb-2">
                                            <div class="row">
                                                ...
                                                <div class="col-md-5">
                                                    <input type="email" class="form-control"
                                                        name="participants[{{ $index }}][email]"
                                                        value="{{ $participant->email }}" placeholder="Email (optional)">
                                                </div>
                                                <div class="col-md-2">
                                                    <button type="button"
                                                        class="btn btn-outline-danger btn-sm remove-participant">
                    ...
                    <div class="timeline">
                        <div class="timeline-item">
                            ...
                            <div class="timeline-content">
                                <h6 class="small mb-1">Booking Created</h6>
                                <p class="small text-muted mb-1">{{ $booking->created_at->format('d/m/Y H:i') }}</p>
                                <p class="small text-muted">by {{ $booking->user->name }}</p>
                            </div>
                        </div>
                        ...
                        @if ($booking->approved_at)
                            <div class="timeline-item">
                                ...
                                <div class="timeline-content">
                                    <h6 class="small mb-1">
                                        {{ $booking->status === 'rejected' ? 'Booking Rejected' : 'Booking Approved' }}
                                    </h6>
                                    <p class="small text-muted mb-1">{{ $booking->approved_at->format('d/m/Y H:i') }}</p>
                                    <p class="small text-muted">by {{ $booking->approver->name ?? 'System' }}</p>
                                    @if ($booking->rejection_reason)
                                        <p class="small text-danger">Reason: {{ $booking->rejection_reason }}</p>
                                    @endif
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($booking->updated_at != $booking->created_at)
                            <div class="timeline-item">
                                ...
                                <div class="timeline-content">
                                    <h6 class="small mb-1">Booking Modified</h6>
                                    <p class="small text-muted mb-1">{{ $booking->updated_at->format('d/m/Y H:i') }}</p>
                                    <p class="small text-muted">Last update</p>
                                </div>
                            </div>
                        @endif
                    </div>
...
@push('scripts')
    ...
    <script>
        let participantIndex = {{ $booking->participants->count() }};
